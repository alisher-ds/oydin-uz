...
(() => {
  ...
  // Lightweight local history. It snapshots the actual persisted map before changes,
  // so Ctrl/Cmd+Z remains useful without coupling this layer to app.js internals.
  if (MAP) {
      ...
      if(!restoring && k==='oydin-maps'){
        try{
          ...
          if(prev && prev!==v){
            ...
            h.push({maps:prev,active:localStorage.getItem('oydin-active-map')});
            localStorage.setItem(HKEY,JSON.stringify(h.slice(-MAX)));
          }
        }catch{}
      }
    ...
    const historyBack=()=>{
      try{
        const h=JSON.parse(localStorage.getItem(HKEY)||'[]');
        const item=h.pop(); if(!item)return;
        restoring=true; originalSet.call(localStorage,HKEY,JSON.stringify(h));
        originalSet.call(localStorage,'oydin-maps',item.maps);
        if(item.active)originalSet.call(localStorage,'oydin-active-map',item.active);
        location.reload();
      }catch{}
    };
    ...
    const addTools=()=>{
      ...
      const wrap=document.createElement('div');wrap.id='oydinProductTools';wrap.className='tool-cluster product-tools';
  ...
  if(OQIM){
    const list=document.querySelector('#ideas'), input=document.querySelector('#ideaInput');
    const addSearch=()=>{
      if(document.querySelector('#oqimSearch'))return;
