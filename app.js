const $=s=>document.querySelector(s);const canvas=$('#canvas'),dialog=$('#cardDialog');let type='G‘oya',zoom=100;
...
let cards=JSON.parse(localStorage.getItem('oydin-cards')||'[]');
...
$('#help').onclick=()=>$('#helpDialog').showModal();$('#closeHelp').onclick=()=>$('#helpDialog').close();const minute=()=>$('#minuteDialog').showModal();$('#openMinute').onclick=minute;$('#openMinuteHero').onclick=minute;$('#closeMinute').onclick=()=>$('#minuteDialog').close();render();
