...
function cardCenter(card){return{x:card.x+102.5,y:card.y+57}}
...
function connect(from,to){
  if(from===to||connections.some(e=>(e.from===from&&e.to===to)||(e.from===to&&e.to===from)))return;
  connections.push({from,to});connectingFrom=null;save();render();
}
function render(){
  canvas.querySelectorAll('.thought-card').forEach(x=>x.remove());
  $('#emptyState').style.display=cards.length?'none':'block';
  $('#count').textContent=`${cards.length} ta fikr`;
  $('#findStep').disabled=!cards.length;
  $('#flowText').textContent=connectingFrom? 'Bog‘lanish uchun boshqa fikrni bosing.':cards.length?`${cards.length} fikr orasidan yo‘nalishni ajrating.`:'Bitta fikr yozing — Oydin siz uchun signalni topadi.';
  cards.forEach(card=>{
    ...
    el.addEventListener('pointerdown',e=>{
      ...
      if(connectingFrom&&connectingFrom!==card.id){e.preventDefault();connect(connectingFrom,card.id);return}
      drag(el,card,e)
    },{passive:false});
  });
  drawConnections();
...
function drag(el,card,startEvent){
  let sx=startEvent.clientX,sy=startEvent.clientY,ox=card.x,oy=card.y;
  el.setPointerCapture(startEvent.pointerId);
  const move=e=>{card.x=Math.max(5,Math.min(canvas.clientWidth-215,ox+e.clientX-sx));card.y=Math.max(55,Math.min(canvas.clientHeight-125,oy+e.clientY-sy));el.style.left=card.x+'px';el.style.top=card.y+'px';drawConnections()};
...
$('#addCard').onclick=open;$('#addFirst').onclick=open;
...
$('#themeToggle').onclick=()=>{const night=document.body.classList.toggle('night');localStorage.setItem('oydin-theme',night?'night':'light');$('#themeToggle').setAttribute('aria-label',night?'Kunduzgi rejimni yoqish':'Tungi rejimni yoqish')};
...
$('#mapTitle').value=localStorage.getItem('oydin-title')||'Bugungi fikrlar';
$('#mapTitle').oninput=()=>{$('#saveStatus').textContent='Saqlanmoqda…';clearTimeout(window.st);window.st=setTimeout(save,350)};
$('#zoomIn').onclick=()=>{zoom=Math.min(150,zoom+10);$('#zoom').textContent=zoom+'%';canvas.style.backgroundSize=(21*zoom/100)+'px '+(21*zoom/100)+'px'};
$('#zoomOut').onclick=()=>{zoom=Math.max(70,zoom-10);$('#zoom').textContent=zoom+'%';canvas.style.backgroundSize=(21*zoom/100)+'px '+(21*zoom/100)+'px'};
$('#help').onclick=()=>$('#helpDialog').showModal();$('#closeHelp').onclick=()=>$('#helpDialog').close();
const minute=()=>$('#minuteDialog').showModal();$('#openMinute').onclick=minute;$('#openMinuteHero').onclick=minute;$('#closeMinute').onclick=()=>$('#minuteDialog').close();
