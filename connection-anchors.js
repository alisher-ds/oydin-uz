...
(() => {
  ...
  const anchor = (el, target, gap = 8) => {
    ...
    const sx = Math.abs(dx) / (w / 2 || 1);
    const sy = Math.abs(dy) / (h / 2 || 1);
  ...
  const redraw = () => {
    ...
    const centers = cards.map(el => ({ el, ...center(el) }));
    groups.forEach(group => {
      ...
      if (!a || !b || a === b) return;
      const p1 = anchor(a.el, { x: b.x, y: b.y });
      const p2 = anchor(b.el, { x: a.x, y: a.y });
      const dx = p2.x - p1.x, dy = p2.y - p1.y, dist = Math.hypot(dx, dy), bend = Math.max(55, Math.min(180, dist * .32));
      const d = Math.abs(dx) > Math.abs(dy)
        ? `M${p1.x},${p1.y} C${p1.x + Math.sign(dx) * bend},${p1.y - dy * .08} ${p2.x - Math.sign(dx) * bend},${p2.y + dy * .08} ${p2.x},${p2.y}`
        : `M${p1.x},${p1.y} C${p1.x - dx * .08},${p1.y + Math.sign(dy) * bend} ${p2.x + dx * .08},${p2.y - Math.sign(dy) * bend} ${p2.x},${p2.y}`;
