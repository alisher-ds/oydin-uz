...
async function requireVault(env, token) {
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    let vault = await requireVault(env, token).catch(() => null);
    const time = now();
    if (!vault) {
      const id = safeId('vault');
      await env.OYDIN_DB.prepare('INSERT INTO vaults (id, token_hash, created_at, updated_at) VALUES (?, ?, ?, ?)').bind(id, await hashToken(token), time, time).run();
      vault = { id };
    }
    ...
    for (const space of spaces.slice(0, 50)) {
      ...
      await env.OYDIN_DB.prepare(`INSERT INTO spaces (id, vault_id, title, data_json, updated_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET title=excluded.title, data_json=excluded.data_json, updated_at=excluded.updated_at WHERE spaces.vault_id=excluded.vault_id AND spaces.updated_at <= excluded.updated_at`).bind(space.id, vault.id, String(space.title || 'Yangi makon').slice(0, 160), data, String(space.updatedAt || time)).run();
    }
    await env.OYDIN_DB.prepare('UPDATE vaults SET updated_at = ? WHERE id = ?').bind(time, vault.id).run();
    const rows = await env.OYDIN_DB.prepare('SELECT data_json FROM spaces WHERE vault_id = ? ORDER BY updated_at DESC').bind(vault.id).all();
    return json({ token, spaces: (rows.results || []).map(r => { try { return JSON.parse(r.data_json); } catch { return null; } }).filter(Boolean), syncedAt: time });
  } catch (error) {
    console.error('Oydin sync error:', error);
    return json({ error: error?.status === 400 || error?.status === 401 ? error.message : 'Sync failed.' }, error?.status || 502);
  }
}
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const vault = await requireVault(env, token);
    if (!vault) return json({ error: 'Vault not found.' }, 404);
    const rows = await env.OYDIN_DB.prepare('SELECT data_json FROM spaces WHERE vault_id = ? ORDER BY updated_at DESC').bind(vault.id).all();
    return json({ spaces: (rows.results || []).map(r => { try { return JSON.parse(r.data_json); } catch { return null; } }).filter(Boolean) });
  } catch (error) {
